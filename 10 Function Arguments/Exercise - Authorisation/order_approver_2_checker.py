...
try:
    from order_approver_2 import check_fissile
except ModuleNotFoundError:
    assert False, "order_approver_2_checker.py tried to import from the file order_approver_2.py but the file didn't exist. Check it is present and the name is spelt correctly."
except ImportError:
    assert False, "order_approver_2_checker.py tried to import the function 'check_fissile' from the file order_approver_2.py. The file existed, but the function didn't. Check the function is in that file and that the name is spelt correctly."
...
try:
    from order_approver_2 import check_poisons
except ModuleNotFoundError:
    assert False, "order_approver_2_checker.py tried to import from the file order_approver_2.py but the file didn't exist. Check it is present and the name is spelt correctly."
except ImportError:
    assert False, "order_approver_2_checker.py tried to import the function 'check_poisons' from the file order_approver_2.py. The file existed, but the function didn't. Check the function is in that file and that the name is spelt correctly."
...
try:
    from order_approver_2 import order_approver
except ModuleNotFoundError:
    assert False, "order_approver_2_checker.py tried to import from the file order_approver_2.py but the file didn't exist. Check it is present and the name is spelt correctly."
except ImportError:
    assert False, "order_approver_2_checker.py tried to import the function 'corder_approver' from the file order_approver_2.py. The file existed, but the function didn't. Check the function is in that file and that the name is spelt correctly."
...
def check_order_approver(path, reference_outcome, reason="", authorisation_code=None, kwargs={}):
    if authorisation_code:
        authorisation_code_valid = authorisation_code in ("UUDDLRLRBAS", "rosebud")

    try:
        # Only supply the authorisation code if one was supplied
        if authorisation_code:
            order_approver_result = order_approver(path, check_fissile, check_poisons, authorisation_code=authorisation_code, **kwargs)
        else:
            order_approver_result = order_approver(path, check_fissile, check_poisons, **kwargs)
    except FileNotFoundError:
        print("order_approver_2 was asked to check the contents of an order found at '{}' but the following FileNotFoundError was raised. Check the file is in the correct location in your computer.".format(path))
        raise
    except ValueError:
        if authorisation_code:
            if authorisation_code_valid:
                authorisation_feedback = "The authorisation code was '{}', which should be valid".format(authorisation_code)
            else:
                return
        else:
    ...
    if authorisation_code:
        assert authorisation_code_valid, "order_approver_2 was asked to check the contents of an order found at '{}' and was provided with the authorisation code '{}' which was invalid. This should have caused a ValueError to be raised, but no ValueError was raised. Check the logic of your code.".format(path, authorisation_code)

    assert not kwargs, "order_approver_2 was asked to check the contents of an order found at '{}' and was provided with the following extra keyword arguments '{}'. These should have been unrecognised and so a ValueError should have been raised, but no ValueError was raised. Check the logic of your code.".format(path, kwargs)

    assert type(order_approver_result) == bool, "order_approver_2 was asked to check the contents of an order found at '{}' but the returned value was of type {} instead of bool. Check your logic.".format(path, type(order_approver_result))

    if reference_outcome:
        error_message = "order_approver_2 was asked to check the contents of an order found at '{}' but the returned value was {} instead of {} as it should be as the file contains no fissile elements or poison compounds. Check your logic.".format(path, order_approver_result, reference_outcome)
    else:
        error_message = "order_approver_2 was asked to check the contents of an order found at '{}' but the returned value was {} instead of {} as it should be because {}. Check your logic.".format(path, order_approver_result, reference_outcome, reason)
...
print("==============Check three orders which should be approved with no authorisation code==================")
check_order_approver(os.path.join(sample_directory_path, "order_empty.txt"), True)

# Check a couple of orders which don't violate any checks cause order_approver_2 to return True
...
# Check a couple of orders which violate one or more checks cause order_approver_2 to return False
print("==============Check three orders which should be rejected with no authorisation code================")
...
# Check a couple of orders which violate one or more checks, but provide a correct authorisation code, which causes order_approver_2 to return True 
print("==============Check three orders which should be rejected, but will be approved with a valid authorisation code================")
check_order_approver(os.path.join(sample_directory_path, "order_3.txt", ), True, authorisation_code="rosebud")
check_order_approver(os.path.join(sample_directory_path, "order_4.txt"), True, authorisation_code="UUDDLRLRBAS" )
check_order_approver(os.path.join(sample_directory_path, "order_5.txt"), True, authorisation_code="rosebud")

# Check that a invalid authorisation code always raises a ValueError
print("==============Check two orders with an invalid authorisation code raise a ValueError================")
check_order_approver(os.path.join(sample_directory_path, "order_3.txt", ), False, authorisation_code="I'm a hacker!")
check_order_approver(os.path.join(sample_directory_path, "order_1.txt"), True, authorisation_code="Rosebud")

# Check that extra keyword arguments always raise a ValueError
print("==============Check eight orders with extra unrecognised keyword arguments raise a ValueError================")
check_order_approver(os.path.join(sample_directory_path, "order_3.txt", ), False, authorisation_code="I'm a hacker!", kwargs={"extra":"Garbage"})
check_order_approver(os.path.join(sample_directory_path, "order_1.txt"), True, authorisation_code="Rosebud", kwargs={"extra":"Garbage"})
check_order_approver(os.path.join(sample_directory_path, "order_4.txt"), True, authorisation_code="UUDDLRLRBAS", kwargs={"extra":"Garbage"})
check_order_approver(os.path.join(sample_directory_path, "order_5.txt"), True, authorisation_code="rosebud", kwargs={"extra":"Garbage"})
check_order_approver(os.path.join(sample_directory_path, "order_1.txt"), True, kwargs={"extra":"Garbage"})
